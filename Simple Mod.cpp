...
string Decryption (string str ,  int key)
{
    ...
    {
        char  c =  str[i] ;
        //  convert  char  to  number by use ASCII code
        int num_c_ASCII =  int(c) ;
        //  find position  number in alphabet
        int pos = num_c_ASCII -  int('a') ;
        // Encryption  number by  equation Ceaser
        // in this equation two  proccess do  first  shifting and  convert number to char
        res +=  (((pos-key) % 26) +'a')  ;
    }
    return  res ;
}
